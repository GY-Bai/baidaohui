...
BEGIN
    ...
    RETURN COALESCE(user_role, 'Fan'); -- 如果用户有id但profiles中没有role，默认为Fan
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
RETURNS BOOLEAN AS $$
DECLARE
    current_user_role TEXT := public.get_current_user_role();
BEGIN
    IF current_user_role IS NOT NULL AND required_roles IS NOT NULL THEN
...
BEGIN
    IF NOT public.is_admin() THEN
        RETURN jsonb_build_object('success', false, 'error', 'insufficient_permissions', 'message', '只有 Master 或 Firstmate 可以调整用户角色');
    END IF;
